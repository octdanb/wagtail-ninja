...
from .helpers import field_middlewares
from .helpers import streamfield_types

from .registry import registry
from .settings import wagtail_ninja_settings
from .schemas.base import BaseSchema
from .schemas.documents import DocumentObjectSchema
from .schemas.images import ImageObjectSchema, ImageRenditionObjectSchema
from .schemas.pages import PageSchema, get_page_interface
from .schemas.rich_text import RichText as RichTextType
from .schemas.snippets import get_snippet_interface
from .schemas.streamfield import generate_streamfield_union


if apps.is_installed("wagtailmedia"):
    from wagtailmedia.models import AbstractMedia

    from .schemas.media import MediaObjectType
...
def import_apps():
    ...
    if isinstance(wagtail_ninja_settings.APPS, (list, tuple)):
        for name in wagtail_ninja_settings.APPS:
            add_app(name)
            registry.apps.append(name)
    else:
        apps = wagtail_ninja_settings.APPS.items()
    ...
    for streamfield_type in streamfield_types:
        ...
        if hasattr(cls, "wagtail_ninja_types"):
            base_type = generate_streamfield_union(cls.wagtail_ninja_types)
...
def get_fields_and_properties(cls):
    ...
    """
    from .model_utils import get_model_fields
...
def build_node_type(
    ...
    type_prefix: str,
    interface,
    base_type: Type[BaseSchema] = BaseSchema,
):
    """
    Build a node type from a model class and associate
    ...
    for list_name in registry.lazy_types:
        ...
        for key, node in type_list.items():
            ...
            if type_definition.get("lazy"):
                ...
                # Recreate the type with the fields set
                class Meta:
                    model = cls
                    interfaces = _interfaces if _interfaces is not None else ()

                type_meta = {"Meta": Meta, "id": int, "name": type_name}
                ...
                for field in get_fields_and_properties(cls):
                    ...
                    # 'Excluding the custom field "<field>" on BaseSchema "<cls>" has no effect.
                ...
                methods = {}
                if hasattr(cls, "wagtail_ninja_fields"):
...
def get_field_value(instance, field_name: str):
    ...
    Different schemas of objects require different ways to access the values.
    """
...
def custom_cls_resolver(*, cls, wagtail_ninja_field):
    klass = cls()

    # If we've defined a `source` kwarg: use it.
    if hasattr(wagtail_ninja_field, "field_source") and hasattr(
        klass, wagtail_ninja_field.field_source
    ):
        if isinstance(getattr(type(cls()), wagtail_ninja_field.field_source), property):
            return lambda self, instance, info, **kwargs: getattr(
                klass, wagtail_ninja_field.field_source
            )
        elif callable(getattr(cls, wagtail_ninja_field.field_source)):
            return lambda self, instance, info, **kwargs: getattr(
                klass, wagtail_ninja_field.field_source
            )(
                info=info,
    ...
    if hasattr(wagtail_ninja_field, "field_name") and hasattr(
        klass, wagtail_ninja_field.field_name
    ):
        if isinstance(getattr(type(cls()), wagtail_ninja_field.field_name), property):
            return lambda self, instance, info, **kwargs: getattr(
                klass, wagtail_ninja_field.field_name
            )
        elif callable(getattr(cls, wagtail_ninja_field.field_name)):
            return lambda self, instance, info, **kwargs: getattr(
                klass, wagtail_ninja_field.field_name
            )(
                info=info,
...
def build_streamfield_type(
    ...
    class Meta:
        if hasattr(cls, "wagtail_ninja_types"):
            types = [
                registry.streamfield_blocks.get(block) for block in cls.wagtail_ninja_types
            ]
        else:
            interfaces = _interfaces if _interfaces is not None else ()
        # Add description to type if the Meta class declares it
        description = getattr(cls._meta_class, "wagtail_ninja_description", None)
    ...
    # Add any custom fields to node if they are defined.
    if hasattr(cls, "wagtail_ninja_fields"):
        for item in cls.wagtail_ninja_fields:
            if callable(item):
                item = item()

            # Get correct schemas from field
            field, field_type = get_field_type(item)

            # Add support for `wagtail_ninja_fields`
            methods[f"resolve_{field.field_name}"] = (
                custom_cls_resolver(cls=cls, wagtail_ninja_fields=field)
    ...
    # Set excluded fields to stop errors cropping up from unsupported field schemas.
    wagtail_ninja_node = type(type_name, (base_type,), type_meta)

    for name, method in methods.items():
        setattr(wagtail_ninja_node, name, MethodType(method, wagtail_ninja_node))

    return wagtail_ninja_node
...
def register_page_model(cls: Type[WagtailPage], type_prefix: str):
    ...
    if page_node_type := build_node_type(cls, type_prefix, get_page_interface(), PageSchema):
...
def register_document_model(cls: Type[AbstractDocument], type_prefix: str):
    ...
    if document_node_type := build_node_type(
        cls, type_prefix, None, DocumentObjectSchema
    ):
...
def register_image_model(cls: Type[AbstractImage], type_prefix: str):
    """
    Create a node type for a model than inherits from AbstractImage.
    ...
    if image_node_type := build_node_type(cls, type_prefix, None, ImageObjectSchema):
...
def register_image_rendition_model(cls: Type[AbstractRendition], type_prefix: str):
    """
    Create a node type for a model than inherits from AbstractImageRendition.
    Only one type will actually be generated because a default image model
    needs to be set in settings.
    """
    ...
    if rendition_type := build_node_type(
        cls, type_prefix, None, ImageRenditionObjectSchema
    ):
...
def register_media_model(cls: Type[AbstractMedia], type_prefix: str):
    """
    Create node type for a model than inherits from AbstractDocument.
    Only one model will actually be generated because a default document model
    needs to be set in settings.
    """
...
def register_settings_model(
    ...
    """
    Create a node type for a settings page.
    """
...
def register_snippet_model(cls: Type[models.Model], type_prefix: str):
    """
    Create a type for a snippet model.
    """
...
def register_django_model(cls: Type[models.Model], type_prefix: str):
    """
    Create a type for (non-specific) Django model.
