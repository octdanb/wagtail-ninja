...
from .schemas.structures import BasePaginatedType, PaginationType


if wagtail_ninja_settings.ADD_SEARCH_HIT:
...
def resolve_site_by_id(
    *,
    ...
    try:
        ...
        # This is an expected error, so should not raise an Exception.
        return None
...
def _sliced_queryset(qs, limit=None, offset=None):
    ...
    # default
    limit = int(limit or wagtail_ninja_settings.PAGE_SIZE)
    # maximum
    limit = min(limit, wagtail_ninja_settings.MAX_PAGE_SIZE)
...
def resolve_queryset(
    ...
    **kwargs,
    ...
    :class:`~wagtail_ninja.schemas.structures.QuerySetList`.
...
def resolve_paginated_queryset(
    ...
    **kwargs,
    ...
    argument names used by
    :function:`~wagtail_ninja.schemas.structures.PaginatedQuerySet`.
    :param qs: The query set to be modified.
    :param info: The info object.
    ...
    page = int(page or 1)
    per_page = min(
        int(per_page or wagtail_ninja_settings.PAGE_SIZE), wagtail_ninja_settings.MAX_PAGE_SIZE
    )
...
def serialize_struct_obj(obj):
    ...
    # This conditional and below support both Wagtail >= 2.13 and <2.12 versions.
